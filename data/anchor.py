...
class Anchor(object):
    ...
    def _generate_anchors(self, feature_map_size, aspect_ratios, scales):
        """
        :param img_size:
        :param feature_map_size:
        :param aspect_ratios:
        ...
        for idx, f_size in enumerate(feature_map_size):
            ...
            for j, i in product(range(int(f_size[0])), range(int(f_size[1]))):
                ...
                x = (i + 0.5) / f_size[1] #* img_size_w 
                y = (j + 0.5) / f_size[0] #* img_size_h
                for ars in aspect_ratios[idx]:
                    for scale in scales[idx]:
                        for ar in ars:
                            a = sqrt(ar)
                            w = scale * a / self.img_size_w
                            h = scale / a / self.img_size_h
